...
function ArmSvc(sessionDetails) {
    ...
    function deleteResourceGroup(sessionDetails) {
        logSvc.info("Deleting Resource Group: ", sessionDetails.resourceGroupName);

        var d = Q.defer();

        _resManClient.resourceGroups.beginDeleteMethod(sessionDetails.resourceGroupName, function (err) {
            if (err) {
                throw new Error(err);
            }
    ...
    function getDeployment(sessionDetails) {
        logSvc.info("Retrieving Deployment...");

        var d = Q.defer();

        _resManClient.deployments.get(sessionDetails.resourceGroupName, "deployment-1", function (err, result) {
            if (err) {
                throw new Error(err);
            }
    ...
    function getDeploymentOperations(sessionDetails) {
        logSvc.info("Retrieving Deployment Operations...");

        var d = Q.defer();

        _resManClient.deploymentOperations.list(sessionDetails.resourceGroupName, "deployment-1", function (err, result) {
            if (err) {
                throw new Error(err);
            }
